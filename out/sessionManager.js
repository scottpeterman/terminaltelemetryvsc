...
class SessionManager {
    constructor(configPath) {
        this.sessionsConfig = { folders: [], credentials: [] };
        this.configPath = '';
        if (configPath) {
        ...
        try {
            if (fs.existsSync(filePath)) {
                ...
                if (Array.isArray(yamlData)) {
                    this.sessionsConfig.folders = yamlData.map((folder, index) => ({
                        id: `folder-${index}`,
                        folder_name: folder.folder_name || `Unnamed Folder ${index}`,
                        sessions: (folder.sessions || []).map((session, sessionIndex) => ({
                            ...session,
    ...
    saveSessionsToFile(filePath) {
        try {
            ...
            const yamlStructure = this.sessionsConfig.folders.map((folder) => ({
    ...
    getFolder(folderId) {
        return this.sessionsConfig.folders.find((folder) => folder.id === folderId);
    }
    // Get all sessions across all folders
    getAllSessions() {
        return this.sessionsConfig.folders.flatMap((folder) => folder.sessions);
    }
    ...
    addSession(folderId, sessionData) {
    ...
    removeFolder(folderId) {
        const folderIndex = this.sessionsConfig.folders.findIndex((folder) => folder.id === folderId);
        if (folderIndex >= 0) {
    ...
    removeSession(sessionId) {
        for (const folder of this.sessionsConfig.folders) {
            const sessionIndex = folder.sessions.findIndex((session) => session.id === sessionId);
            if (sessionIndex >= 0) {
